...
import net.dachi.dream.poi.ModPOIs;
...
public class ModTeleporter implements ITeleporter {
    ...
    public static DreamPortalBlock portal;


    public ModTeleporter(ServerWorld worldIn) {
        this.world = worldIn;
    }
    public Optional<TeleportationRepositioner.Result> getExistingPortal(BlockPos pos, boolean isNether) {
        ...
        Optional<PointOfInterest> optional = pointofinterestmanager.getInSquare((poiType) -> {
            return poiType == ModPOIs.DREAM_PORTAL.get();
            ...
            for(int l1 = -1; l1 < 2; ++l1) {
                for(int k2 = 0; k2 < 2; ++k2) {
                    for(int i3 = -1; i3 < 3; ++i3) {
                        BlockState blockstate1 = i3 < 0 ? ModBlocks.DREAM_BLOCK.get().getDefaultState() : Blocks.AIR.getDefaultState();
        ...
        for(int k1 = -1; k1 < 3; ++k1) {
            for(int i2 = -1; i2 < 4; ++i2) {
                if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
                    ...
                    this.world.setBlockState(blockpos$mutable, ModBlocks.DREAM_BLOCK.get().getDefaultState(), 3);
                }
            }
        }

        BlockState blockstate = ModBlocks.DREAM_PORTAL_BLOCK.get().getDefaultState().with(DreamPortalBlock.AXIS, axis);
