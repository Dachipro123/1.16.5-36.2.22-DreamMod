...
public class ModPortalSize {
    ...
    public ModPortalSize(IWorld worldIn, BlockPos pos, Direction.Axis axisIn) {
        this.world = worldIn;
        this.axis = axisIn;
    ...
    private int func_242969_a(BlockPos.Mutable mutablePos) {
        for(int i = 0; i < 21; ++i) {
            ...
            for(int j = 0; j < this.width; ++j) {
                ...
                if (blockstate.matchesBlock(ModBlocks.DREAM_PORTAL_BLOCK.get())) {
    ...
    public static boolean canConnect(BlockState state) {
        return state.isAir() || state.isIn(BlockTags.FIRE) || state.matchesBlock(ModBlocks.DREAM_PORTAL_BLOCK.get());
    }
    ...
    public void placePortalBlocks() {
        BlockState blockstate = ModBlocks.DREAM_PORTAL_BLOCK.get().getDefaultState().with(DreamPortalBlock.AXIS, this.axis);
