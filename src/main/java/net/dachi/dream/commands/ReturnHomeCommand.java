...
public class ReturnHomeCommand {
    ...
    private int returnHome(CommandSource source) throws CommandSyntaxException {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray(Dream.MOD_ID + "homepos").length != 0;

        if(hasHomepos) {
            int[] playerPos = player.getPersistentData().getIntArray(Dream.MOD_ID + "homepos");
